 ...
 * entirely on-device. This file is the FALLBACK that keeps every screen
 * working when a real BLE adapter isn't reachable — which is the normal case
 * in Expo Go (the native `react-native-ble-plx` module can't load there) and
 * on web. The real reader lives in `elm327.ble.ts`; `elm327.ts` is the facade
 * that picks between them at runtime.
 *
 * For the demo we generate realistic readings entirely on-device so dispatch
 * is no longer dependent on Herath's predictive-maintenance service being up.
 *
 * Design properties:
...
export function pair(vehicleId: string): PairingInfo {
  pairing = {
    mac:       generateFakeMac(),
    pairedAt:  Date.now(),
    vehicleId,
...
/**
 * Read live OBD-II PIDs from the paired (simulated) ELM327.
