...
#
# `load_dotenv()` lives in app/database.py, so reading these when this module
# is first imported only works if database.py happened to be imported first.
# It is, through app.main - but anything importing this module directly saw an
# unconfigured key and silently fell back to the deterministic text, which is
# indistinguishable from the feature being switched off. A disabled feature
# should be a decision, not an import-order accident.
def _api_key() -> str:
...
async def explain(req: ExplainRequest) -> ExplainResponse:
    ...
    try:
        ...
        return ExplainResponse(text=text, source="llm", model=_model())
